...
export default function IncomeTaxResult({
  numAmount,
  appliedExpense,
  taxableIncome,
  estimatedTax,
  alreadyPaid,
  additionalTax,
  isRefund: isRefundProp,
}: IncomeTaxResultProps) {
  ...
  const resultBg = isRefund
    ? isDark
      ? tintBg(colors.success[500], 0.12)
      : colors.success[50]
    : isDark
    ? tintBg(colors.danger[500], 0.12)
    : colors.danger[50];
  ...
  return (
    <Card className="mx-5 mt-5 p-5">
      <View className="flex-row items-center gap-2 mb-4">
        <View
          ...
          style={{
            backgroundColor: isDark
              ? tintBg(colors.success[500], 0.15)
              : colors.success[100],
          }}
        >
          <Ionicons
        ...
        <Text style={{ color: t.text }} className="text-lg font-bold">
          종소세 예상 결과
        </Text>
      </View>
      ...
      <StatRow
        label="적용 경비"
        value={`${appliedExpense.toLocaleString()}원`}
      />
      <StatRow label="과세표준" value={`${taxableIncome.toLocaleString()}원`} />
      <StatRow label="산출 세액" value={`${estimatedTax.toLocaleString()}원`} />
      <StatRow
        label="기납부 원천징수"
        value={`${alreadyPaid.toLocaleString()}원`}
        showBorder={false}
      />
      <View
        className="flex-row justify-between items-center mt-4 rounded-2xl p-4"
